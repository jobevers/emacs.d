...
def _getText(lines, start_line, end_line):
    result = []
    for index in range(start_line, end_line):
        result.append(lines.get_line(index))
    return '\n'.join(result)


def _getNodeText(pymodule, node):
    lines = pymodule.logical_lines
    end_line = lines.logical_line_in(node.lineno)[1] + 1
    return _getText(pymodule.lines, node.lineno, end_line)


def _isNodeMonkey(pymodule, node):
    text = _getNodeText(pymodule, node)
    return text.find('monkey') >= 0
...
class ModuleImports(object):
    ...
    def get_changed_source(self):
        ...
        result = []
        # Writing module docs and monkey imports
    ...
    def _first_import_line(self):
        # this is where the imports SHOULD go, not where they are
        # returns the first line after the docstring, or 0 if there is no
        # docstring
        ...
        while True:
            ...
            if _isNodeMonkey(self.pymodule, node):
                monkey_found = True
                starting_node += 1
            else:
                break
        if len(nodes) > starting_node:
            lineno = self.pymodule.logical_lines.logical_line_in(
                nodes[starting_node].lineno)[0]
        else:
        ...
        while lineno > 1:
            ...
            if line.strip() == '':
                ...
                break
        if monkey_found:
            # add a line here to seperate the monkey imports
...
class _GlobalImportFinder(object):
    ...
    def _get_text(self, start_line, end_line):
        _getText(self.lines, start_line, end_line)
    ...
    def visit_from(self, node, end_line):
        ...
        if node.level:
            level = node.level
        start_line = node.lineno
        for name in self._get_names(node.names):
            import_info = importinfo.FromImport(
                node.module, level, [name])
            self.imports.append(importinfo.ImportStatement(
                import_info, node.lineno, end_line, None,
    ...
    def find_import_statements(self):
        ...
        for index, node in enumerate(nodes):
            if isinstance(node, (ast.Import, ast.ImportFrom)):
                ...
                end_line = lines.logical_line_in(node.lineno)[1] + 1
                if first_import and _isNodeMonkey(self.pymodule, node):
                    skip = True
                first_import = False
                if skip:
                    continue
